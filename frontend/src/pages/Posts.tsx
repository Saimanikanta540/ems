...
// API response type for backend posts
...
const Posts = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const backendPosts: PostApiResponse[] = await api.get('/posts');
        // Adapt backend model to frontend Post shape if needed
        const mapped = backendPosts.map((p: PostApiResponse): Post => ({
          id: String(p.id),
          title: p.title,
          ...
          tags: Array.isArray(p.tags) ? p.tags.map((t: { name?: string } | string) => typeof t === 'string' ? t : t.name || String(t)) : [],
